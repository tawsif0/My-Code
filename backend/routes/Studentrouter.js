...
Studentrouter.put(
  ...
  async (req, res) => {
    try {
      ...
      // Return complete student data for frontend update
      res.status(200).json({
        success: true,
        ...
        profile_picture: req.file?.filename // Keep this for backward compatibility
      });
    } catch (error) {
      res.status(500).json({
        success: false,
...
Studentrouter.put("/profile/:id/password", studentAuth, async (req, res) => {
  try {
    ...
    // Set the new password and save with validation disabled
    student.password = newPassword;
    await student.save({ validateBeforeSave: false }); // This will trigger the pre-save hook but skip validation
...
Studentrouter.get("/certificate/:courseId/:studentId", async (req, res) => {
  try {
    ...
    doc.fontSize(10).text("Director of Education", pageWidth - 250, sigY + 45, {
      align: "left"
    });
