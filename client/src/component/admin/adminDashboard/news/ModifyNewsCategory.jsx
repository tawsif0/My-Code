...
function ModifyNewsCategory() {
  ...
  useEffect(() => {
    ...
    const handleCategoryUpdate = () => {
      console.log("News category updated event received");
      fetchCategories();
    };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:3500/api/news-categories/${id}`, {
        headers: authHeaders,
      });
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await axios.put(
        `http://localhost:3500/api/news-categories/${id}`,
        { name: editName },
        {
          headers: {
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">
              News Category List & Modifications
            </h1>
            <p className="text-gray-600 mt-2">
              Modify the categories for organizing news articles
            </p>
          </div>
          <button
...
export default ModifyNewsCategory;